...
from api.app.services.db import get_cfg, get_statuts_col, get_ejustice_col
...
async def statuts_stream(bce: str):
    ...
    ejustice_col = get_ejustice_col()
    bce_fmt = normalize_bce(bce)

    async def generator():
        async for msg in stream_statuts_sse(bce_fmt, cfg, statuts_col, ejustice_col):
            yield msg
